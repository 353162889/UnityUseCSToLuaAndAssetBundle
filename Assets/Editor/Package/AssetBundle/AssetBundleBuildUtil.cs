...
namespace EditorPackage
{
    public class AssetBundleBuildUtil
    {
        ...
        [MenuItem("Test/TestDeleteLuaTemp")]
        //删除lua的临时目录与文件
        private static void DeleteLuaTemp()
        {
            PathTools.RemoveDir(PathConfig.LuaBytesRootDir);
            AssetDatabase.Refresh();
        }
        [MenuItem("Test/TestCopyLuaToTemp")]
        //创建lua的临时目录与文件
        private static void CopyLuaToLuaTemp()
        {
            PathTools.CopyToRenameExtension(PathConfig.LuaRootDir, PathConfig.LuaBytesRootDir, "*.lua", ".lua.bytes");
            AssetDatabase.Refresh();
        }
